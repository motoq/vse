...
/**
 * Jacobian relating the complex portion of a quaternion transformation
 * from a unit pointing vector in one reference frame to another.
 * 
 * @author   Kurt Motekew
 * @since    20130921
 */
public class DuvDqC extends Matrix {
  private Matrix3X3 body2Sensor = new Matrix3X3();
  private Matrix dpdb = new Matrix(2,3);             // pointing to body
  private Matrix dbdi = new Matrix(3,3);             // body to computational

  /**
   * Instantiate as a [2x3] matrix with all partials set to zero.
   */
  public DuvDqC() {
    super(2,3);
  }
   ...
   *  element is 'v'.
   *  <P>
   *  This class differs from DuvDq in that only the partials w.r.t. the
   *  complex components are computed.
   *
   ...
   *                 taken w.r.t.
   *
   * @return         A pointer to this Jacobian relating u & v to  the complex
   *                 components of q
   *                        <P>
   *                                  -                -
   *                           dpdq = | du/dq1  dv/dq1 |
  ...
  public Matrix partials(Tuple3D xyz, Quaternion qbs, Quaternion q) {
    ...
    dbdi.put(1,1, 4*x*q1 + 2*y*q2 + 2*z*q3);
    dbdi.put(1,2, 2*y*q1 - 2*z*q0);
    dbdi.put(1,3, 2*z*q1 + 2*y*q0);
      // Row 2
    dbdi.put(2,1, 2*x*q2 + 2*z*q0);
    dbdi.put(2,2, 2*x*q1 + 4*y*q2 + 2*z*q3);
    dbdi.put(2,3, 2*z*q2 - 2*x*q0);
      // Row 3
    dbdi.put(3,1, 2*x*q3 - 2*y*q0);
    dbdi.put(3,2, 2*y*q3 + 2*x*q0);
    dbdi.put(3,3, 2*x*q1 + 2*y*q2 + 4*z*q3);
